...
class FavoriteRepository {
  ...
  Future<List<Ad>> getFavorites(User user) async {
    ...
    if(response.success && response.results != null) {
      return response.results.map((po) => Ad.fromParse(po.get(keyFavoritesAd)));
  ...
  Future<void> save({Ad ad, User user}) async {
      ...
      favoriteObject.set<String>(keyFavoritesOwner, user.id); // traz todos os favoritos no usuario

      favoriteObject.set<ParseObject>(keyFavoritesAd,
        ParseObject(keyAdTable)..set(keyAdId, ad.id) // cria relação entre as tabelas
      );
  ...
  Future<void> delete({Ad ad, User user}) async {
    ...
    queryBuilder.whereEqualTo(keyFavoritesOwner, user.id);
    queryBuilder.whereEqualTo(keyFavoritesAd, ParseObject(keyAdTable)..set(keyAdId, ad.id));
